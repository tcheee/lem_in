...
/*   Updated: 2019/03/02 10:57:32 by tcherret         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (*j < farm->nb_path)
	{
		...
		while (k > 0)
		{
			if (farm->room[farm->path[*j].path[k - 1]].ant > 0)
			{
				...
				ft_printf("L%d-%s ",
						farm->room[farm->path[*j].path[k]].ant_name,
						farm->room[farm->path[*j].path[k]].name);
			}
			k--;
		}
		(*j)++;
...
int				moving_display(t_farm *farm)
{
	...
	while (graph_finished(farm) != 1)
	{
		farm->flow = 0;
		i = 0;
		while (farm->room[start].ant >= farm->flow && i < farm->nb_path)
			farm->flow += farm->path[i++].weight;
		j = 0;
		while (j < farm->nb_path)
			loop_algo(farm, &j);
		farm->counter++;
